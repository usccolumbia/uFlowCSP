...
# Usage: sbatch train_meanflow_raw_csp_ordered_formula_atomwise.sh [flow_ratio] [cfg_scale] [split_type] \
#                [sym_lattice] [sym_rotavg] [sym_anchor] [sym_loss_weight] \
#                [atom_ordering] [perm_augment] [modulo_translation] [use_formula_embedding] [use_atomwise_features]
#
...
#
# Example (formula + atomwise embedding on, ordering-only A/B vs baseline):
#   sbatch train_meanflow_raw_csp_ordered_formula_atomwise.sh 0.25 1.5 difCSP false false false 0.0 symmetry false false true true
# Example (atomwise off, for direct A/B against train_meanflow_raw_csp_ordered_formula.sh):
#   sbatch train_meanflow_raw_csp_ordered_formula_atomwise.sh 0.25 1.5 difCSP false false false 0.0 symmetry false false true false
#
# ============================================================================
#  This is a NEW, ADDITIVE variant of train_meanflow_raw_csp_ordered_formula.sh,
#  which itself is unchanged by this file. The only difference here is the
#  denoiser: MeanFlowSiTRawFormulaAtomwise additionally concatenates fixed,
#  non-trainable per-token chemistry features (Pauling electronegativity,
#  atomic radius, period, group -- see src/models/denoisers/atomic_features.py)
#  onto each atom's own token, alongside its learned Z-embedding:
#
#    token_input = [Z-embedding | atomic chemistry features | coords | lattice]
#
#  This is on top of (not instead of) the existing formula-global composition
#  embedding e_formula in c = e_t + e_r + e_formula. It does not require or
#  reintroduce spacegroup (SG) as a required inference input.
#
#  With use_atomwise_features=false this reproduces
#  train_meanflow_raw_csp_ordered_formula.sh's conditioning exactly (still
#  routed through the new denoiser class/config, but with the per-token
#  chemistry features disabled).
#
#  All configs/splits/defaults below are otherwise identical to
#  train_meanflow_raw_csp_ordered_formula.sh -- unchanged.
# ============================================================================
...
application="$PYTHON $REPO_ROOT/src/train_diffusion.py --config-path=$REPO_ROOT/configs --config-name=train_meanflow_raw_formula_atomwise_cfg_supervised"
...
#! MCFlow-style atom ordering flags (same as train_meanflow_raw_csp_ordered_formula.sh)
...
#! Formula-global composition embedding flag (same as train_meanflow_raw_csp_ordered_formula.sh).
...
#! NEW: per-token atomic chemistry feature flag (this variant's whole point).
#  Default true; set to false (12th positional arg or ATOMWISE_EMBED_OVERRIDE)
#  to A/B against MeanFlowSiTRawFormula's conditioning while still using this script.
...
name="${run_name_prefix}train_meanflow_raw_${RUN_DATE}_csp_cfgscale${cfg_scale}_fr${flow_ratio}_${jvp_api}_split${split_type}_CFG${cfg_dropout}${sg_suffix}${alex_suffix}${sym_suffix}${ord_suffix}${formula_suffix}${atomwise_suffix}"
...
task_name=train_meanflow_raw_csp_ordered_formula_atomwise
...
if [ -n "${CAMPAIGN_LABEL:-}" ]; then
    CAMPAIGN_RUN_DIR="${workdir}/logs/train_meanflow_raw_csp_ordered_formula_atomwise/runs/campaign_${CAMPAIGN_LABEL}"

...
# Usage: sbatch train_meanflow_raw_csp_ordered.sh [flow_ratio] [cfg_scale] [split_type] \
#                [sym_lattice] [sym_rotavg] [sym_anchor] [sym_loss_weight] \
#                [atom_ordering] [perm_augment] [modulo_translation]
#
...
#
# Example (symmetry ordering, ordering-only A/B vs baseline):
#   sbatch train_meanflow_raw_csp_ordered.sh 0.25 1.5 difCSP false false false 0.0 symmetry false false
# Example (simple EN ordering + permutation augmentation):
#   sbatch train_meanflow_raw_csp_ordered.sh 0.25 1.5 difCSP false false false 0.0 simple true true
#
# ============================================================================
#  MCFlow-style composition/symmetry-aware atom ordering for raw MeanFlow CSP.
#
#  This is a NEW, ADDITIVE variant of train_meanflow_raw_csp.sh. It only changes
#  the ORDER in which atoms are presented to the order-sensitive transformer
#  (plus optional symmetry-preserving permutation augmentation). The MeanFlow
#  objective and model architecture are unchanged. With atom_ordering=none this
#  reproduces the legacy pipeline exactly. Ordered runs use a SEPARATE dataset
#  cache (mp20_raw_ordered_*.pt / alex_raw_ordered_*.pt) so existing caches and
...
application="$PYTHON $REPO_ROOT/src/train_diffusion.py --config-path=$REPO_ROOT/configs --config-name=train_meanflow_raw_cfg_supervised"
...
use_spacegroup=${USE_SPACEGROUP:-true}  # true: SG-conditioned training; false: force null SG everywhere
...
#! MCFlow-style atom ordering flags (NEW)
...
name="${run_name_prefix}train_meanflow_raw_${RUN_DATE}_csp_cfgscale${cfg_scale}_fr${flow_ratio}_${jvp_api}_split${split_type}_CFG${cfg_dropout}${sg_suffix}${alex_suffix}${sym_suffix}${ord_suffix}"
...
trainer.max_epochs=${MAX_EPOCHS:-10000}
+trainer.precision=bf16-mixed
logger=wandb
logger.wandb.name=${name}
logger.wandb.project=${WANDB_PROJECT}
task_name=train_meanflow_raw_csp_ordered
...
if [ -n "${CAMPAIGN_LABEL:-}" ]; then
    CAMPAIGN_RUN_DIR="${workdir}/logs/train_meanflow_raw_csp_ordered/runs/campaign_${CAMPAIGN_LABEL}"

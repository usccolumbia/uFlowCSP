"""
MeanFlowSiTRawFormula: MeanFlowSiTRaw + an explicit formula-global composition embedding.

This is a NEW, ADDITIVE copy of meanflow_sit_raw.py. It does not modify the
original MeanFlowSiTRaw class or any of its behavior; existing configs/checkpoints
that reference MeanFlowSiTRaw are completely unaffected.

Motivation
----------
For formula-only CSP we do not want to reintroduce spacegroup (SG) as a required
inference input. Instead we inject symmetry-compatible information that is
derivable purely from the formula: a global composition vector built from
normalized element counts, added into the conditioning vector alongside the
existing time embeddings:

    c = e_t + e_r + e_formula   (+ e_spacegroup, if SG conditioning is still enabled)

Right now the base model only sees composition indirectly, via attention over
repeated per-atom-type tokens. e_formula gives the model the whole composition
explicitly and up front, independent of sequence length/order.
"""
...
class MeanFlowSiTRawFormula(nn.Module):
    """
    Sequence Transformer for MeanFlow on raw atomic data, with an optional
    formula-global composition embedding added to the conditioning vector.

    Identical to MeanFlowSiTRaw except for the `use_formula_embedding` option
    (default True in this class) and the resulting e_formula term in `c`.
    ...
    Args:
        ...
        use_formula_embedding: Whether to compute and add e_formula to `c`.
                            Default True (this class exists specifically to
                            enable it); set False to reproduce MeanFlowSiTRaw's
                            conditioning exactly while still using this class.
    """
    ...
    def __init__(
        ...
        self.model_input_dim = atom_type_embed_dim + self.in_channels
        ...
        # NEW: formula-global composition embedder. Consumes a normalized
